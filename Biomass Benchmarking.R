...
######### MorFi PCA Sugargraze ONLY ########################

morfi_benchmarking_pub <- read_csv("morfi_benchmarking_publication.csv")


morfi_benchmarking_PCA <- subset(morfi_benchmarking_pub,select = -c(Biomass,Section,Level,Morfi.Rep))
...
PCA.benchmarking.biomass <- c(rep("Banana",24),rep("Softwood",12),rep("Sugarcane",24),rep("Spinifex",12),rep("Sorghum",48))
PCA.benchmarking.biomass <- factor(PCA.benchmarking.biomass, levels = c("Banana","Softwood","Sugarcane","Spinifex","Sorghum"))
...
tiff("Fig7.tiff", units="in", width=9.86, height=7.96, res=300)

pca_biomass <- ggbiplot(PCA.benchmarking,varname.size = 3,
                        obs.scale = 0.5, 
                        ellipse=TRUE, 
                        ellipse.prob = 0.68, 
                        alpha = 0.5, 
                        groups = PCA.benchmarking.biomass,
                        color="black") +
...
#################################### Clustering - Benchmarking Publication ##############

#Hierarchical Clustering - All Biomass

morfi_benchmarking_pub$biomass_ID <- with(morfi_benchmarking_pub,paste(Biomass,Section, sep= "-"))

morfi_benchmarking_means <- morfi_benchmarking_pub %>% group_by(biomass_ID) %>% dplyr::summarise(across(.cols=where(is.numeric),mean,na.rm = TRUE))  
morfi_benchmarking_means <- morfi_benchmarking_means %>% dplyr:: select(1,3:23)
...
tiff("Fig 8.tiff", units="in", width=7.96, height=7.96, res=300)
...
benchmarking_biomass_morfi_nanopaper_corr <- cor(morfi_benchmarking_nonwood_PCA_means[,1:21],nanopaper_benchmarking_biomass_PCA_means[,5:9])  
...
############ Sustainability Triangle - BENCHMARKING PUBLICATION ##############

nanopaper_benchmarking_biomass_pub <- read_csv("nanopaper_benchmarking_biomass_publication.csv")


#Organising Tensile Index (TI) parameters


nanopaper_benchmarking_esr.TI <- dplyr::select(nanopaper_benchmarking_biomass_pub,c(1:3,15))
...
ggplot(nanopaper_benchmarking_esr.TI) +
  aes(x = Level, y = TI_norm, color = Biomass, shape = Section) +
  scale_color_manual(name = 'Biomass', values=c("darkgoldenrod1","chartreuse2","brown", "darkgreen")) +
  scale_shape_manual(values=c(15,17,16,1,10,3,4)) + 
  theme_bw() + 
  geom_point() + 
  stat_summary(aes(group=ID), fun=mean, geom="line", colour="green") +
...
tiff("Fig9.tiff", units="in", width=9.86, height=7.96, res=300)
...
Biomass_Benchmarking_TGA_Symbols_Publication <- subset(Biomass_Benchmarking_TGA_Symbols,subset=(Biomass == 'Sorghum'|
                                                                                  Biomass =='Softwood'|
                                                                                  Biomass =='Sugarcane'|
                                                                                  Biomass =='Banana'|
                                                                                  Biomass == 'Spinifex'))
...
tiff("Fig 3.tiff", units="in", width=10, height=5, res=300)

p <- ggplot(subset(Biomass_Benchmarking_TGA_Symbols_Publication, Metric %in% c("Step"))) +
  aes(x=Biomass, y=Value) + 
  geom_point(size=5, aes(shape = Section, color = Metric)) +   # Draw points
  scale_shape_manual(values=c(12,
                              15,
...
p <- ggplot(subset(Biomass_Benchmarking_TGA_Symbols_Publication, Metric %in% c("Onset"))) +
  aes(x=Biomass, y=Value) + 
  geom_point(size=5, aes(shape = Section, color = Metric)) +   # Draw points
  scale_shape_manual(values=c(12,
                              15,
...
p <- ggplot(subset(Biomass_Benchmarking_TGA_Symbols_Publication, Metric %in% c("Residue"))) +
  aes(x=Biomass, y=Value) + 
  geom_point(size=5, aes(shape = Section, color = Metric)) +   # Draw points
  scale_shape_manual(values=c(12,
                              15,
...
############ Crystallinity _ Benchmarking Publication ##########################
...
crystallinity_Publication <- subset(crystallinity,subset=(Biomass == 'Sorghum'|
                                                            Biomass =='Softwood'|
                                                            Biomass =='Sugarcane'|
                                                            Biomass =='Banana'|
                                                            Biomass == 'Spinifex'))

tiff("test1.tiff", units="in", width=10, height=5, res=300)

p <- ggplot(subset(crystallinity_Publication)) +
  aes(x=Biomass, y=Crystallinity) + 
  geom_point(size=5, aes(shape = Type, color = Biomass)) +   # Draw points
  scale_shape_manual(values=c(12,
                              15,

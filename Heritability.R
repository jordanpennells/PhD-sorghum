...
fits.coef.morfi_quality_mean <- fits.coef.morfi_quality %>% dplyr::group_by(parameter) %>% dplyr::mutate(mean=mean(slope)) %>% dplyr::mutate(parameter_order = fct_reorder(parameter, mean,.fun='mean'))


v <- ggplot(fits.coef.morfi_quality_mean, aes(fct_reorder(parameter_order,mean), y =slope, fill=parameter))
v + geom_violin(size = 1) + 
  labs(title="Violin plot", 
       subtitle="MorFi Parameter vs Quality Gradient (Q1-7)",
       x="MorFi Parameter",
       y="Quality Gradient") +
  coord_flip() + 
  # gghighlight(max(value) > 20)

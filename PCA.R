...
tiff("Fig6.5.tiff", units="in", width=11.35, height=6.25, res=300)

fviz_eig(PCA.test) + theme(plot.title = element_blank(),
...
tiff("Fig6.4b.tiff", units="in", width=6.6, height=6.25, res=300)
...
p_variety + theme(panel.grid.minor = element_blank(),
                  panel.grid.major = element_blank(),
...
tiff("Fig6.4c.tiff", units="in", width=6.6, height=6.25, res=300)
...
tiff("PCA_thesis defence.tiff", units="in", width=7.37, height=7.96, res=300)

p_HPH12 <- ggbiplot(PCA.test,choices=1:2, 
                    varname.size = 4,
                    obs.scale = 0.35, 
                    ellipse=TRUE, 
                    ellipse.prob = 0.68, 
                    alpha = 1, 
                    groups = PCA.test.HPH) + 
  scale_color_manual(name = 'HPH Energy Level', 
                     values=c("lightgreen", "orange", "red")) + 
...
p_HPH12 + theme(legend.title = element_text(face = "bold", size = 14), 
                legend.text = element_text(size=12), 
                axis.title = element_text(face = "bold", size = 14),
                axis.text = element_text(size = 12),
                panel.grid.minor = element_blank(),
...
nanopaper <- read_csv("nanopaper_clean.csv")


# Datacamp tutorial _ Nanopaper_all metrics

nanopaper_PCA <- subset(nanopaper,select = -c(X1,ID,sample_name,rep,variety,section,HPH,nanopaper_rep,strain,UTS,modulus,toughness,TI))
...
tiff("Fig5.11a.tiff", units="in", width=11.5, height=8, res=300)
...
p_nanopaper_HPH + theme(axis.title.x = element_text(size=14, face="bold"),
...
tiff("Fig5.11c.tiff", units="in", width=9, height=6, res=300)
...
p_nanopaper_section + theme(axis.title.x = element_text(size=14, face="bold"),
                            axis.text.x = element_text(size = 14),
                            axis.title.y = element_text(size=14, face="bold"),
                            axis.text.y = element_text(size = 14),
                            legend.title = element_text(size = 14),
...
tiff("Fig5.11b.tiff", units="in", width=9, height=6, res=300)
...
p_nanopaper_variety + theme(axis.title.x = element_text(size=14, face="bold"),
                            axis.text.x = element_text(size = 14),
                            axis.title.y = element_text(size=14, face="bold"),
                            axis.text.y = element_text(size = 14),
                            legend.title = element_text(size = 14),
